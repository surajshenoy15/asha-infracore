...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://asha-infracore-backend.onrender.com/api/products')
      ...
      .then(data => {
        ...
        const found = data.find(p => slugify(p.name) === productId);
        setProduct(found);

        if (found) {
          const firstImage = [found.image1, found.image2, found.image3, found.image4].find(Boolean);
    ...
    try {
      ...
      if (!product.features) {
        ...
        if (
          ...
          product.features.includes('[object Object]')
        ) {
          console.warn('Invalid features format:', product.features);
          parsed = [];
        } else {
          parsed = JSON.parse(product.features);
        }
      ...
      parsed = parsed.filter(
        item => item && typeof item === 'object' && item.title && typeof item.title === 'string'
      );
  ...
  const similarModels = allProducts.filter(
    (p) => p.category === product?.category && p.id !== product?.id
  );
  const visibleSimilar = similarModels.slice(currentSimilarIndex, currentSimilarIndex + 3);
...
>
  Request a Quote
          ...
          <div className="max-w-7xl mx-auto px-4 relative">
            <div className="text-center mb-16">
              ...
              <h2 className="text-5xl font-black text-gray-900 mb-6 leading-tight">
                Why Choose the <span className="text-[#FF3600]">{product.name}</span>?
              </h2>
              <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                Discover the advanced features that make this machine the perfect choice for your projects
              </p>
            </div>
            ...
            {product.spec_pdf_url ? (
              ...
              >
                <p className="text-lg font-semibold flex items-center gap-3">
                  ...
                  Download {product.name} Specifications PDF
                </p>
              </a>
            ) : (
              <div className="block bg-gradient-to-r from-gray-300 to-gray-400 rounded-2xl p-6 text-white opacity-60 cursor-not-allowed">
                <p className="text-lg font-semibold flex items-center gap-3">
                  <Download className="w-6 h-6" />
                  Specifications PDF not available for this product
                </p>
              </div>
            )}
          </div>
      ...
      {/* Simplified Similar Models Section */}
      {similarModels.length > 0 && (
        <div className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex justify-between items-center mb-12">
              <div>
                <h2 className="text-4xl font-bold text-gray-900 mb-2">Similar Models</h2>
  ...
  const basePath = product?.category?.toLowerCase().includes('mini-excavator')
    ? 'mini-excavators'
    : 'loaders';
  const slug = similar.name.toLowerCase().replace(/\s+/g, '-');

  return (
    <Link
      key={similar.id}
      to={`/${basePath}/${similar.category}/${slug}`}
    ...
    >
      <div className="overflow-hidden">
        <img
          src={similar.image1 || '/placeholder.jpg'}
          alt={similar.name}
          ...
          {similar.description}
                ...
                <button
                  onClick={() => setCurrentSimilarIndex(i => {
                    const nextIndex = i + 3;
                    const maxIndex = Math.max(0, Math.floor((similarModels.length - 1) / 3) * 3);
                    return Math.min(nextIndex, maxIndex);
                  })}
                  disabled={currentSimilarIndex + 3 >= similarModels.length}
                  className="flex-shrink-0 p-4 rounded-full bg-white shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 hover:scale-110"
                >
              ...
              <div className="flex justify-center mt-8 gap-2">
                {Array.from({ length: Math.ceil(similarModels.length / 3) }).map((_, idx) => (
                  <button
                    key={idx}
